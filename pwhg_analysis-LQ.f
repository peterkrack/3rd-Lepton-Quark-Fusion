...
c This was after recombination; better be before ...
     ...
     1                    + phep(1:4,iphoton(j))
         ...
         enddo         
         do i=2,nleps
            if (abs(phep(4,ilep(i))).gt.0) then
               call getdydetadphidr(phep(1:4,ilep(1)),
     1              phep(1:4,ilep(i)),dylph,detalph,dphilph,drlph)
               if (drlph.lt.0.2) then
                  phep(1:4,ilep(1)) = phep(1:4,ilep(1))
     1             + phep(1:4,ilep(i))
                  phep(1:4,ilep(i))=0d0
               endif 
            endif
...
C     Fill histograms
     ...
     1 pt1 .gt. ptcutlep .and. ptjet1 .gt. ptcutjet .and.
     2 mrec .gt. mLQcutlo .and. mrec .lt. mLQcuthi .and. 
     3 mjets .gt. 0 .and. nleps .gt. 0 .and. dr.gt.2d0) then
