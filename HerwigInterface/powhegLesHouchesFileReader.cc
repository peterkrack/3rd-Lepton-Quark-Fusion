...
void powhegLesHouchesFileReader::open() {
  ...
  for ( int i = 0; i < max_num_weights; ++i )
    {
      weights_.weight[i]=0;
      string wname=to_string(i+1);
      optionalWeightsNames.push_back(wname);
    }
  weights_.radtype=0;
...
bool powhegLesHouchesFileReader::doReadEvent() {
  if (hepevt_.nevhep>=maxev){
      std::cout<<"Stopping execution by returning an error in the powhegLesHouchesFileReader. "<<std::endl;
      std::cout<<"This is not an error, but a trick to get the analysis output done."<<std::endl;	 
      return false;
  } 
  ...
  while ( cfile.readline() && !cfile.find("</event>")) {
    ...
    if(line.compare("<weights>")==0)
      {
        optionalWeights["W1"]=hepeup.XWGTUP;
	int i=0;
	while ( cfile.readline() && !cfile.find("</weights>"))
	  {
	    cfile >> weights_.weight[i];
            string weightName=to_string(i+2);
            weightName='W'+weightName;
	    optionalWeights[weightName] = weights_.weight[i];
            i++;
